...
% Thrust
rdd = qd{qn}.acc_des + diag(Kv)*(qd{qn}.vel_des - qd{qn}.vel) + diag(Kp)*(qd{qn}.pos_des - qd{qn}.pos);

F= mass*(g + rdd(3));

phi_des = (mass/F)*(rdd(1)*sin(qd{qn}.yaw_des) - ...
    rdd(2)*cos(qd{qn}.yaw_des));
theta_des = (mass/F)*(rdd(1)*cos(qd{qn}.yaw_des) + ...
    rdd(2)*sin(qd{qn}.yaw_des));

alt_des = [phi_des; theta_des; qd{qn}.yaw_des];
omega_des = [0; 0; qd{qn}.yawdot_des];
