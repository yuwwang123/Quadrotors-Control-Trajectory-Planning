...
%% Recover the path 
path = goal;
current_ind = parent_inds(current(1));
...
function [Nodes, N, status, parent_inds]= add_neighbors(map, Nodes, N, node, status, parent_inds, goal, astar)
    ...
    %                   Nodes_ind = Nodes(1,:);
    %                   local_ind = find(Nodes_ind==neighbor(1));
    %                   Nodes(5,local_ind) = min(Nodes(5,local_ind), node(5)+norm(node_pos-neighbor_pos));
    %                   Nodes(7,local_ind) = Nodes(5,local_ind) + Nodes(6,local_ind);
...
function  [neighbor_ind ,neighbor_cost] = get_neighbor(map, u_ind, bound_max, step)
    u_pos = ind2pos(map, u_ind);
    neighbor_pos = u_pos + map.res_xyz./2 + step * diag(map.res_xyz);
    neighbor_sub = pos2sub(map, neighbor_pos);
    exceed_bound = (sum((neighbor_sub - [1 1 1]) >= 0, 2) == 3 & sum((neighbor_sub - bound_max) <= 0, 2 ) == 3)==0;
    neighbor_pos(exceed_bound, :) = [];
    step(exceed_bound, :) = [];
    neighbor_ind = pos2ind(map, neighbor_pos);
    collision = map.occgrid(neighbor_ind) == 1;
    neighbor_ind(collision, :) = [];
    step(collision, :) = [];
    neighbor_cost = step * diag(map.res_xyz);
end
...
% end
% end
function h = compute_H(u_pos, goal, astar)
    if astar
        delta = abs((u_pos - goal));
        [a, I] = min(delta);
        delta(I) = [];
        delta = delta - a;
        [b, I] = min(delta);
        delta(I) = [];
        delta = delta - b;
        h = 1.7321 * a + 1.414 * b + delta; 

%        h = norm(u_pos - goal);
%        h = sum(abs(u_pos - goal));
    else
        h = 0;
    end
end
